...
const DemoHeader = ({
  ...
  <div className="px-5 md:px-7 py-3 flex items-center justify-between border-b border-white/[0.04]">
    ...
    <div className="flex items-center gap-2">
      <button
        className="w-7 h-7 rounded-md hover:bg-shimmer/40 flex items-center justify-center text-[#9b99a0]"
        disabled
      >
        <Search size={13} />
      </button>
          ...
          >
            ...
            <div className="col-span-2">
              <span
                ...
                style={{ background: 'rgba(122,164,255,0.10)', color: '#7aa4ff' }}
              >
                {r.intent}
              </span>
            </div>
            <div className="col-span-2 flex items-center justify-end gap-2">
              <div className="flex-1 max-w-[60px] h-1 rounded-full bg-shimmer/60 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${r.score}%`,
                    background: 'linear-gradient(90deg, #7aa4ff55, #abc7ff)',
                  }}
                />
              </div>
    ...
    <div className="flex-1 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto fo-thin-scroll px-5 md:px-7 py-3">
        {rows.map((r, i) => (
          ...
          >
            <div
              className="w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-semibold text-[#0a0a0c]"
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[10px] text-[#7e7c83]">
                {r.role} ／ {r.co}
              </div>
            </div>
            <span
              className="text-[10px] font-mono px-2 py-0.5 rounded"
              style={{ background: `${r.col}1a`, color: r.col }}
            >
              {r.sig}
            </span>
          </div>
    ...
    <div className="flex-1 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto fo-thin-scroll px-5 md:px-7 py-3">
        ...
        {rows.map((r, i) => (
          <div
            key={i}
            className="grid grid-cols-12 items-center text-[12px] py-2"
            style={{ borderTop: i === 0 ? 'none' : '1px solid rgba(171,199,255,0.05)' }}
          >
            ...
            <div className="col-span-1">
              <span className="text-[10px] font-mono" style={{ color: r.col }}>
                ● {r.s}
              </span>
            </div>
    ...
    <div className="flex-1 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto fo-thin-scroll px-5 md:px-7 py-3 space-y-1.5">
        {tasks.map((t, i) => (
          <div
            key={i}
            className="flex items-center gap-3 px-3 py-2 rounded-md bg-pitch fo-glass-rim"
          >
            <span
              className="w-3.5 h-3.5 rounded border flex-shrink-0 flex items-center justify-center"
              style={{
                borderColor: t.done ? '#8dffc9' : '#414753',
                background: t.done ? '#8dffc922' : 'transparent',
              }}
            >
              {t.done && (
                <span className="text-[8px]" style={{ color: '#8dffc9' }}>
                  ✓
                </span>
              )}
            </span>
            <div className="flex-1 min-w-0">
              <div
                className={`text-[12.5px] ${t.done ? 'text-[#7e7c83] line-through' : 'text-[#e7e5ea]'}`}
              >
                {t.t}
              </div>
            ...
            <span
              ...
              style={{
                background:
                  t.p === 'high'
                    ? 'rgba(255,141,207,0.12)'
                    : t.p === 'mid'
                      ? 'rgba(255,207,74,0.10)'
                      : 'rgba(155,153,160,0.08)',
              ...
              style={{
                background:
                  r.status === '有人'
                    ...
                    : r.status === '解決'
                      ? 'rgba(141,255,201,0.10)'
                      : 'rgba(171,199,255,0.10)',
                color:
                  r.status === '有人' ? '#ffcf4a' : r.status === '解決' ? '#8dffc9' : '#abc7ff',
              }}
            >
              {r.status}
            </span>
          ...
          {cells.map((cell, i) => (
            <div key={i} className="rounded-lg bg-pitch p-3 fo-glass-rim relative overflow-hidden">
              <div
                ...
                style={{
                  background: `radial-gradient(circle, ${cell.color}22, transparent 70%)`,
                  filter: 'blur(12px)',
                }}
              />
              <div className="text-[11px] text-[#9b99a0] relative">{cell.t}</div>
              <div
                className="font-display font-bold text-[1.6rem] mt-1 relative"
    ...
    <div className="flex-1 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto fo-thin-scroll px-5 md:px-7 py-3 space-y-1.5">
        {seqs.map((s, i) => (
          <div
            key={i}
            className="flex items-center gap-3 px-3 py-2.5 rounded-md bg-pitch fo-glass-rim"
          >
            <span
              className="text-[10px] font-mono px-2 py-0.5 rounded"
              style={{ background: `${s.col}1a`, color: s.col }}
            >
              {s.st}
            </span>
            <div className="flex-1 text-[12.5px] text-[#e7e5ea] truncate">{s.t}</div>
            <div className="text-[10px] font-mono text-[#9b99a0]">
              配信 <span className="text-[#e7e5ea]">{s.lead}</span>
            </div>
            <div className="text-[10px] font-mono text-[#9b99a0]">
              開封 <span className="text-aurora">{s.open}%</span>
            </div>
            <div className="text-[10px] font-mono text-[#9b99a0]">
              CTR <span className="text-mint">{s.ctr}%</span>
            </div>
          </div>
        ))}
      </div>
        ...
        {items.map((it, i) => (
          <div key={i} className="rounded-lg bg-pitch p-3 fo-glass-rim">
            <div className="flex items-center justify-between">
              ...
              <span
                className="text-[10px] font-mono px-1.5 py-0.5 rounded"
                style={{ background: 'rgba(255,107,141,0.12)', color: '#ff6b8d' }}
              >
                {it.cnt}社
              </span>
            </div>
            <div className="mt-2 flex items-center gap-3 text-[10px] font-mono text-[#9b99a0]">
              ...
              <div className="flex-1 h-1 rounded-full bg-shimmer/40 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${it.score}%`,
                    background: 'linear-gradient(90deg, #ff6b8d55, #ff6b8d)',
                    boxShadow: '0 0 6px #ff6b8d80',
                  }}
                />
              </div>
...
export const KnowledgeDemo = () => {
  const tickets = [
    {
      q: '経費精算の上限額って、出張先で違いましたっけ？',
      a: '国内 8,000円/日 / 海外は地域A〜C…',
      conf: 95,
    },
            ...
            style={
              i === 0
                ? {
                    ...
                    color: '#c8b9ff',
                    boxShadow: 'inset 0 0 0 1px rgba(200,185,255,0.20)',
                  }
                : { color: '#7e7c83' }
            }
          >
      ...
      <div className="flex-1 overflow-y-auto fo-thin-scroll px-5 md:px-7 py-3 space-y-2">
        {tickets.map((tk, i) => (
          <div key={i} className="rounded-lg bg-pitch p-3 fo-glass-rim">
            ...
            <div className="mt-1.5 text-[11px] text-[#9b99a0]">
              <span className="text-mint font-mono mr-1">A.</span>
              {tk.a}
            </div>
          </div>
        ))}
...
export const SearchDemo = () => (
  <div className="flex-1 flex flex-col items-center justify-center px-8 py-16 text-center">
    ...
    <div className="font-display font-bold text-[1.4rem] mt-5 fo-gradient-text">
      企業・コンタクト・取引・議事録を横断検索
    </div>
    ...
    <div className="mt-5 flex flex-wrap gap-2 justify-center">
      {['#テクノリード', '#PoC', '#見積', '#Q2', '#Salesforce 連携'].map((t) => (
        <span
          key={t}
          className="text-[11px] px-2.5 py-1 rounded-full bg-pitch fo-glass-rim text-[#c7c5c9]"
        >
          {t}
        </span>
      ))}
